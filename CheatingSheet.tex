...
\ifXeTeX
\else
  \errmessage{CheatingPaper.tex must be compiled with XeLaTeX}
\fi
...
\IfFileExists{fonts/SFCompactText-Regular.ttf}{}
  {\renewcommand{\CheatFontPath}{CheatingSeetTemplate/fonts/}}
...
正文使用 5 pt 字号、6 pt 行间距；英文字体为 SF Compact Text，中文字体为苹方-简。%
将这一段替换为你的 Cheating Paper 内容。行内公式用 \verb|\m{...}| 或 \verb|\tightmath{...}|；

还可以使用像这样的单行公式：
$$\mathrm{Re} = \frac{\text{惯性力}}{\text{黏性力}} = \frac{\rho Vl}{\mu} = \frac{Vl}{\nu}$$

多字母语义标识使用 \verb|\mathrm{...}| 或 \verb|\operatorname{...}|，如 \m{P+\mathrm{SSR}}、\m{P_t^{\mathrm{ex}}}、\m{\operatorname{Cov}(x,y)}；
真正的变量乘积保持数学写法，如 \m{xy} 或 \m{X'P_ZX}。

\divider

\begin{itemize}
  \item \pointtitle{要点标题保持正文大小并使用 SemiBlack}，颜色使用 \#0076BA。
  \item 公式示例：\tightmath{a^2+b^2=c^2}
\end{itemize}
